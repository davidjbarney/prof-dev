...
All of the examples in this guide will use the [2016 CCES](https://doi.org/10.7910/DVN/GDF6Z0) loaded in `.dta` format. To replicate, simply update the relative filepath below to the location of your local version of the dataset.
...
```
```{r include=FALSE}
rm(sg_bp, subgroup_vars, subgroup_matrix, subgroup_df, subgroup_means)
```
...
```{r Dummy out vars for weighting}
cces16$female <- NA
